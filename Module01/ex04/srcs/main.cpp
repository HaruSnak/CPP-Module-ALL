...
/*   Updated: 2025/04/12 16:58:49 by shmoreno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char *argv[])
{
	...
	if (argc != 4 || argv[2][0] == '\0')
		ft_error_base("Three parameters in the following order: a filename and two strings, s1 and s2 (No Empty)." + color::COLOR_NONE);
	std::ifstream file(argv[1]);
	if (!file)
		ft_error_base("Invalid file!" + color::COLOR_NONE);
	outputName = argv[1];
	outputName += ".replace";
